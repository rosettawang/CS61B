...
public class IntList {
    ...
    static IntList dcatenate(IntList A, IntList B) {
        IntList aLastValue = A;
        if (A == null){
            return B;
        }
        //make aLastValue point to tail of A
        while(aLastValue.tail != null){
            aLastValue = aLastValue.tail;
        }
        //make tail of A point to B
        aLastValue.tail = B;
        return A; // REPLACE WITH YOUR CODE */
        //rescursive
        //return dcatenate(A.tail, B), B)
    ...
    static IntList subTail(IntList L, int start) {
        ...
        return position; // REPLACE WITH YOUR CODE
    }
    ...
    static IntList sublist(IntList L, int start, int len) {
        IntList LSubTail = subTail(L, start);
        IntList result = null;
        for (;len > 0; len --){
            //traverse to end
            IntList marker = LSubTail;
            for (int i = len; i > 1; i --){
                marker = marker.tail;
            }
            int last = marker.head;
            result = new IntList(last, result);
        }

        /** prefer recursive version
        if(start < 0 || len <= 0)
            return null;
        else {
            if (start == 0)
    ...
    static IntList dsublist(IntList L, int start, int len) {
        IntList result = subTail(L, start);
        if (len == 0)
            return null;
        IntList marker = result;
        //traverse to end
        for (int i = len; i > 1; i --){
            marker = marker.tail;
        }
